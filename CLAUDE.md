...
- 2026-09-23 어드민 캠페인 로그 엑셀 추출 (services/export_service.py): 주문 관리 화면의 필터(상태·채널·매체·기간·검색 + **계정(user)·등록일 from/to**)를 그대로 쓰는 GET /admin/orders/export, 체크한 캠페인만 뽑는 POST(ids). 시트 3장 — 캠페인(1행=1건, 상품ID·ID 종류·플레이스ID·환불(−)·순매출) / 일별 로그(캠페인×날짜, 예정 수량 + campaign_daily 기록 수량·순위) / 주별 정산(월요일 시작 주 × 계정 × 채널 × 매체, 이용 일수·수량·금액, **환불은 credit_ledger 환불 시점 주에 별도 행 −**). 이용 일수는 running/done/stopped 캠페인의 실제 구동일(시작~min(종료, 오늘, 중단일))만 센다. 일별·주별 활동 기간은 from/to, 없으면 최근 31일. **상품ID 주의**: 스마트스토어·브랜드스토어 URL에서 뽑는 건 스토어 상품번호이고 진짜 nvMid 는 search.shopping.naver.com/catalog|product/ 또는 ?nvMid= 일 때만 — 순위 연동(docs/RANK_INTEGRATION.md 2단계) 붙이면 nvMid 저장으로 보완. naver.me 단축 주소는 플레이스ID 추출 불가.
