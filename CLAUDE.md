...
- 2026-09-21 인기 트래픽 개편 (기준 문서: Downloads/prototype_popular_traffic.html — 프로토타입이 디자인 정본): /popular 페이지 + 대시보드 위젯 + 공용 드로어. 프로토타입 CSS를 static/css/popular.css 로 옮기되, 위젯이 대시보드에도 올라가서 :root 를 쓰면 전역 토큰을 덮어쓰므로 **토큰은 --p-* , 클래스는 p- 접두사**로 두고 .poppage/.p-widget/.p-drawer/.p-scrim 에만 선언한다.
  - media(=ad_types)에 origin(own/ready) / group_key(reward/inflow) / no_refund_days / rank_lead_days / op_note / rating_avg / review_cnt 추가. origin·group_key 는 group_name 에서 유도해 백필했다. 카드 얼굴 색은 origin·group_key 로 정한다 (자체 reward 보라 / 자체 inflow 파랑 / 기성 검정).
  - 운영팀 주간 추천은 weekly_ranks(week_start, channel, type_id, rank 1~3), 주 시작은 월요일(models/weekly_rank.week_start).
  - 후기는 reviews. 그 상품으로 status=done 인 캠페인이 있어야 쓸 수 있고 **완료 캠페인당 1건**(campaign_id UNIQUE). 등록 시 캠페인의 키워드·기간을 복사하고 닉네임은 nick_service.draw() 로 뽑아 고정 저장한다. rating_avg/review_cnt 는 models/review.recount() 로만 갱신하고 목록 조회에서 집계하지 않는다. 후기 0건이면 카드·위젯에서 평점 영역을 통째로 비운다.
  - 드로어는 페이지·위젯 공용. macros/traffic.html 의 drawer_body 를 GET /popular/drawer/<id> 가 렌더해 주고 static/js/popular.js 가 받아 넣는다. 설명·한줄평은 어드민 입력값이라 시드에 넣지 말 것 (NULL 이면 "설명이 아직 등록되지 않았습니다").
  - 채널 탭·정렬 모두 클라이언트 전환(세 채널 36개를 전부 렌더). 현재 채널은 history.replaceState 로 ?ch= 에 반영하고, 탭은 &lt;a href&gt; 라 JS 없이도 동작한다.
  - 콘텐츠 폭은 .poppage max-width 1164px — 프로토타입 .page(1220px, border-box, 좌우 패딩 28px)의 실제 콘텐츠 폭과 같다. 이 값에서 카드 그리드가 프로토타입과 같은 5칸이 된다.
  - 매체별 익명 댓글(media_comments/media_nicks, nick_service.pick_media)은 2026-09-21 폐기. 후기로 대체됐고 라우트·모델·템플릿·테이블 전부 제거(migrate 가 DROP).
